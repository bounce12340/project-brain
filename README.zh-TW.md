...
3. 「任務」四視圖自由切換；看板支援階段與卡片拖拉；點任務開抽屜編輯內容、負責人、起訖日、依賴，並使用留言 `@提及`、附件與 AI 摘要。里程碑／歷程事件只填到期日或事件日時維持單點；加填選填結束日後，日曆會在期間內每日標記，甘特則畫成帶雙菱形端點的橫條。完成任務使用斜紋，逾期未完成任務與里程碑在右端顯示紅色標記。
...
| v13.2 | 里程碑／歷程事件支援選填日期範圍；甘特放大字級並新增完成斜紋與逾期端標 | [SPEC](SPEC-V13-2.md) / [驗收](ACCEPTANCE-V13-2.md) |
