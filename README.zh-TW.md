...
6. 進度保存成功後，AI 可能建議把明確完成的既有任務標為完成、建立後續任務或未來里程碑，以及替既有未完成任務設定日期。建議本身絕不寫資料：所有 checkbox 預設空白，只有人工勾選再按「套用所選」才會呼叫既有任務／里程碑端點；建議的標題與日期都可先修改。歷史日期只留在進度敘事，不建立物件。可在個人設定關閉進度連動建議（`AIUR_PROGRESS_LINKS`，預設開）。
