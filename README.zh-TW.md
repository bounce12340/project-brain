...
5. 進度保存成功後，AI 可能建議把明確完成的既有任務標為完成，或建立後續任務。建議本身絕不寫資料：所有 checkbox 預設空白，只有人工勾選再按「套用所選」才會呼叫既有任務 PATCH／POST；新增任務的標題、階段與到期日都可先修改。可在個人設定關閉「進度紀錄後顯示任務建議」（`AIUR_PROGRESS_LINKS`，預設開）。
