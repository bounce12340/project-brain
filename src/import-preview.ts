...
export function createImportPreview(source: string): ImportPreview {
  ...
  const projects = (body.projects ?? []) as Array<{ tasks?: unknown; milestones?: unknown; events?: unknown; progress_updates?: unknown }>;
  const count = (item: unknown) => Array.isArray(item) ? item.length : 0;
  return {
    projects: projects.length,
    tasks: projects.reduce((sum, item) => sum + count(item.tasks), 0),
    milestones: projects.reduce((sum, item) => sum + count(item.milestones), 0),
    events: projects.reduce((sum, item) => sum + count(item.events), 0),
