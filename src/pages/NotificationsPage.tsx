...
export function NotificationsPage() {
  ...
  return <><PageHeader title="通知中心" description="里程碑、待辦、BD 與停滯專案提醒。" actions={<button className="btn-secondary" onClick={() => void api("/notifications/read-all", { method: "POST" }).then(load)}>全部標為已讀</button>} />{items.length ? <div className="space-y-3">{items.map((item) => <button className={`panel w-full text-left ${item.read ? "opacity-60" : "border-psi-deep"}`} key={item.id} onClick={() => void open(item)}><div className="flex justify-between gap-3"><h2 className="font-semibold">{!item.read && <span className="mr-2 text-psi">●</span>}{item.title}</h2><span className="text-xs text-star-dim">{formatDate(item.created_at, true)}</span></div><p className="mt-2 text-sm text-star-dim">{item.body}</p></button>)}</div> : <Empty>目前沒有通知</Empty>}</>;
}
