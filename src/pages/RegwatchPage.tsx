...
export function RegwatchPage() {
  ...
  const load = () => api<{ entries: RegEntry[]; page: number; total: number; total_pages: number; can_manage: boolean }>(`/regwatch?${query}`).then(setData).catch((cause) => setError(cause instanceof Error ? cause.message : t("regwatch.loadFailed")));
  ...
  return <><PageHeader title={t("nav.regwatch")} description={t("regwatch.description")} actions={data.can_manage && <div className="flex gap-2"><button className="btn-secondary" onClick={() => setAiOpen(true)}>{t("regwatch.aiImport")}</button><button className="btn" onClick={() => setEditing("new")}>{t("regwatch.new")}</button></div>} />
    ...
    <section className="space-y-3">{data.entries.map((item) => { const open = expanded.has(item.id); return <article className="panel" key={item.id}><button className="w-full text-left" onClick={() => setExpanded((current) => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next; })}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs text-star-dim">{item.entry_date} · {t(item.entry_type === "announcement" ? "regwatch.announcement" : "regwatch.meeting")}</p><h2 className="mt-1 font-bold">{item.title}</h2><div className="mt-2 flex gap-2"><span className="badge">{item.product_line}</span>{item.category && <span className="badge">{item.category}</span>}</div></div><span className="text-psi">{t(open ? "common.collapse" : "common.expand")}</span></div></button>{open && <div className="mt-4 border-t border-nexus-line pt-4"><p className="whitespace-pre-wrap text-sm leading-7 text-star-dim">{item.key_points || t("regwatch.noPoints")}</p><div className="mt-3 flex flex-wrap gap-4">{item.link && <a className="text-sm text-psi underline" href={item.link} target="_blank" rel="noreferrer">{t("regwatch.openSource")}</a>}{item.file_id && <a className="text-sm text-psi underline" href={`/api/files/${item.file_id}/download`}>{t("regwatch.downloadSource")}</a>}</div><p className="mt-3 text-xs text-star-dim">{t("common.createdBy", { name: item.created_by_name })}</p>{data.can_manage && <div className="mt-4 flex gap-3"><button className="btn-secondary" onClick={() => setEditing(item)}>{t("common.edit")}</button><button className="btn-danger" onClick={() => void remove(item)}>{t("common.delete")}</button></div>}</div>}</article>; })}{data.entries.length === 0 && <Empty>{t("regwatch.empty")}</Empty>}</section>
...
function AiImportDrawer({ onClose, onImported }: { onClose(): void; onImported(): Promise<void> | void }) {
  ...
  const extract = async (event: FormEvent) => {
    ...
    try {
      ...
      if (sourceMode === "file") {
        if (!file) throw new Error(t("regwatch.fileRequired"));
