...
export function ProfilePage() {
  ...
  return <><PageHeader title="個人設定" description={`${user?.name} · ${user?.email} · ${user?.group_name}`} />{error && <ErrorBox message={error} />}{message && <div className="mb-4 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">{message}</div>}<div className="grid gap-6 md:grid-cols-2"><section className="card"><h2 className="mb-4 font-bold">Email 通知</h2><label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} />接收每日提醒彙整信</label><button className="btn mt-5" onClick={() => void saveNotice()}>儲存偏好</button></section><form className="card" onSubmit={password}><h2 className="mb-4 font-bold">變更密碼</h2><label className="label">目前密碼</label><input className="mb-3 w-full" type="password" value={current} onChange={(e) => setCurrent(e.target.value)} required /><label className="label">新密碼（至少 8 碼）</label><input className="mb-4 w-full" type="password" minLength={8} value={next} onChange={(e) => setNext(e.target.value)} required /><button className="btn">變更密碼</button></form></div><p className="mt-6 text-sm text-slate-500">若您被要求強制變更密碼，請前往 <Link className="text-brand-600" to="/change-password">強制改密頁</Link>。</p></>;
}
