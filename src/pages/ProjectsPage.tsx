...
export function ProjectsPage() {
  ...
  return <><PageHeader title="專案" description="依組別、狀態或關鍵字尋找專案。" actions={user?.role !== "intern" && <button className="btn" onClick={() => setShowNew(!showNew)}>＋ 新增專案</button>} />
    ...
    <div className="panel mb-5 grid gap-3 md:grid-cols-3"><select value={filters.group} onChange={(e) => setFilters({ ...filters, group: e.target.value })}><option value="">全部組別</option>{meta?.groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select><select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">全部狀態</option><option value="active">進行中</option><option value="paused">暫停</option><option value="done">完成</option><option value="archived">歸檔</option></select><input placeholder="搜尋專案…" value={filters.keyword} onChange={(e) => setFilters({ ...filters, keyword: e.target.value })} /></div>
    {!projects ? <Loading /> : projects.length ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{projects.map((project, index) => <Link data-tour={index === 0 ? "project-card" : undefined} to={`/projects/${project.id}`} className="panel transition hover:shadow-[0_0_16px_rgba(53,200,255,.28)]" key={project.id}><div className="mb-4 flex items-start justify-between gap-2"><div><span className="badge mb-2">{project.group_name}</span><h2 className="font-bold">{project.visibility === "private" && "🔒 "}{project.name}</h2></div><span className="text-xs text-star-dim">{statusLabel[project.status]}</span></div><p className="mb-4 line-clamp-2 min-h-10 text-sm text-star-dim">{project.description || "尚無說明"}</p><ProgressBar value={project.progress} /><div className="mt-4 flex justify-between text-xs text-star-dim"><span>Owner：{project.owner_name}</span><span>目標：{project.target_date || "—"}</span></div></Link>)}</div> : <Empty>找不到符合條件的專案</Empty>}
  </>;
}
