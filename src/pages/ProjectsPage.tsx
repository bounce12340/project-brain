...
function NewProject({ metadata, onDone }: { metadata: Metadata; onDone(): void }) {
  const t = useT(); const [form, setForm] = useState({ name: "", description: "", group_id: metadata.groups[0]?.id ?? "", visibility: "group", template_id: "", target_date: "", product: "", site: "" }); const [error, setError] = useState("");
