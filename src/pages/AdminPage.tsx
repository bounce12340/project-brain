...
interface ImportReport { projects: { created: number; updated: number }; tasks: { created: number; skipped: number }; progress_updates: { created: number; skipped: number }; reg_entries: { created: number; skipped: number }; warnings: string[] }
...
export function AdminPage() {
  ...
  const tabs = [["registrations", `註冊申請${registrations.length ? ` (${registrations.length})` : ""}`], ["transfer", "管理權移轉"], ["users", "使用者"], ["groups", "組別"], ["templates", "階段模板"], ["import", "批次匯入"], ["audit", "稽核紀錄"]];
  ...
  return <>
    ...
    {tab === "registrations" && <div className="space-y-5"><section className="panel flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-bold">開放自助註冊</h2><p className="mt-1 text-sm text-star-dim">關閉後公開頁仍可查看，但不接受驗證碼與新申請。</p></div><button role="switch" aria-checked={registrationEnabled} className={`relative h-7 w-12 rounded-full transition ${registrationEnabled ? "bg-psi-deep" : "bg-nexus-line"}`} onClick={() => void toggleRegistration()}><span className={`absolute top-1 h-5 w-5 rounded-full bg-nexus transition ${registrationEnabled ? "left-6" : "left-1"}`} /></button></section><section className="panel overflow-x-auto"><h2 className="mb-4 font-bold">待核清單</h2>{registrations.length === 0 ? <p className="empty-inline text-sm text-star-dim">目前沒有待核准申請。</p> : <table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b border-nexus-line text-gold-bright"><th className="py-2">姓名／Email</th><th>申請時間</th><th>角色</th><th>組別</th><th>操作</th></tr></thead><tbody>{registrations.map((item) => <tr className="border-b border-nexus-line" key={item.id}><td className="py-3"><p className="font-medium">{item.name}</p><p className="text-xs text-star-dim">{item.email}</p></td><td>{formatDate(item.created_at, true)}</td><td><select value={item.role} onChange={(event) => updateRegistration(item.id, { role: event.target.value as "member" | "intern" })}><option value="member">正職成員</option><option value="intern">實習生</option></select></td><td><select value={item.group_id} onChange={(event) => updateRegistration(item.id, { group_id: event.target.value })}>{meta?.groups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select></td><td className="space-x-3"><button className="font-semibold text-ok" onClick={() => void action(() => api(`/admin/registrations/${item.id}/approve`, { method: "POST", body: JSON.stringify({ role: item.role, group_id: item.group_id }) }), "申請已核准並寄送通知")}>核准</button><button className="font-semibold text-danger" onClick={() => { if (window.confirm(`確定拒絕 ${item.name} 的申請？`)) void action(() => api(`/admin/registrations/${item.id}/reject`, { method: "POST" }), "申請已拒絕並寄送通知"); }}>拒絕</button></td></tr>)}</tbody></table>}</section></div>}

    {tab === "transfer" && <form className="panel max-w-3xl space-y-5" onSubmit={submitTransfer}><div><h2 className="text-lg font-bold">管理權移轉</h2><p className="mt-1 text-sm text-star-dim">指定已啟用且已核准的成員接任；執行前必須驗證你的目前密碼。</p></div><div><label className="label">接班人</label><select className="w-full" value={successorId} onChange={(event) => setSuccessorId(event.target.value)} required disabled={!candidates.length || transferBusy}><option value="">請選擇接班人</option>{candidates.map((candidate) => <option value={candidate.id} key={candidate.id}>{candidate.name}（{candidate.email}／{candidate.group_name}／{candidate.role === "intern" ? "實習生" : "正職成員"}）</option>)}</select>{!candidates.length && <p className="empty-inline mt-2 text-sm text-warn">目前沒有符合資格的接班人。</p>}</div><fieldset className="space-y-3"><legend className="label">移轉模式</legend><label className="flex cursor-pointer items-start gap-3 rounded-lg border border-nexus-line p-4"><input className="mt-1" type="radio" name="transfer_mode" value="co_admin" checked={transferMode === "co_admin"} onChange={() => setTransferMode("co_admin")} /><span><strong>升為共同管理員（建議）</strong><span className="mt-1 block text-sm text-star-dim">接班人升為管理員，你仍保有管理權，適合日常備援。</span></span></label><label className="flex cursor-pointer items-start gap-3 rounded-lg border border-danger p-4"><input className="mt-1" type="radio" name="transfer_mode" value="full_transfer" checked={transferMode === "full_transfer"} onChange={() => setTransferMode("full_transfer")} /><span><strong>完全移轉</strong><span className="mt-1 block text-sm text-danger">接班人升為管理員後，你會立即降為正職成員。</span></span></label></fieldset><div><label className="label">你的目前密碼</label><input className="w-full" type="password" autoComplete="current-password" value={transferPassword} onChange={(event) => setTransferPassword(event.target.value)} required /></div><button className={transferMode === "full_transfer" ? "btn-danger" : "btn"} disabled={!successorId || !transferPassword || transferBusy}>{transferBusy ? "移轉中…" : "確認移轉"}</button></form>}

    {tab === "users" && <div className="space-y-6"><form className="panel grid gap-3 md:grid-cols-5" onSubmit={createUser}><input name="name" placeholder="姓名" required /><input name="email" type="email" placeholder="Email" required /><select name="group_id">{meta?.groups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select><select name="role"><option value="member">正職成員</option><option value="intern">實習生</option><option value="admin">管理員</option></select><input name="password" placeholder="初始密碼" minLength={8} required /><button className="btn md:col-start-5">建立帳號</button></form><section className="panel overflow-x-auto"><div className="mb-3 flex items-center justify-between"><h2 className="font-bold">使用者清單</h2><span className="text-sm text-star-dim">啟用且已核准的管理員：{activeAdminCount}</span></div><table className="w-full min-w-[780px] text-left text-sm"><thead><tr className="border-b border-nexus-line text-gold-bright"><th className="py-2">姓名</th><th>Email</th><th>組別</th><th>角色</th><th>狀態</th><th>操作</th></tr></thead><tbody>{users.map((user) => { const isLastAdmin = user.role === "admin" && user.is_active === 1 && user.approval_status === "approved" && activeAdminCount === 1; const reason = isLastAdmin ? "系統至少需要一名管理員" : undefined; const deactivateReason = reason ?? (user.id === currentUser?.id ? "不可停用目前登入帳號" : undefined); return <tr className="border-b border-nexus-line" key={user.id}><td className="py-3">{user.name}</td><td>{user.email}</td><td>{user.group_name}</td><td>{user.approval_status === "approved" ? <select aria-label={`${user.name}角色`} value={user.role} title={reason} onChange={(event) => void changeRole(user, event.target.value)}><option value="member" disabled={isLastAdmin}>正職成員</option><option value="intern" disabled={isLastAdmin}>實習生</option><option value="admin">管理員</option></select> : user.role}</td><td>{user.approval_status === "pending" ? "待核准" : user.approval_status === "rejected" ? "未通過" : user.is_active ? "啟用" : "停用"}</td><td className="space-x-2">{user.approval_status === "approved" ? <><button className="text-psi" onClick={() => void resetPassword(user.id)}>重設密碼</button>{user.is_active ? <span title={deactivateReason}><button className="text-danger" disabled={isLastAdmin || user.id === currentUser?.id} onClick={() => void action(() => api(`/admin/users/${user.id}`, { method: "DELETE" }), "帳號已停用")}>停用</button></span> : <button className="text-ok" onClick={() => void action(() => api(`/admin/users/${user.id}`, patchBody({ is_active: true })), "帳號已啟用")}>啟用</button>}</> : <button className="text-danger" onClick={() => { if (window.confirm(`確定刪除 ${user.email}？`)) void action(() => api(`/admin/users/${user.id}`, { method: "DELETE" }), "未核准帳號已刪除"); }}>刪除</button>}</td></tr>; })}</tbody></table></section></div>}
