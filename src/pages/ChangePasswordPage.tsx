...
export function ChangePasswordPage() {
  ...
  return <main className="grid min-h-screen place-items-center bg-nexus-raised px-4"><form onSubmit={submit} className="panel w-full max-w-md !p-8"><h1 className="mb-2 text-2xl font-bold">設定新密碼</h1><p className="mb-6 text-sm text-star-dim">首次登入或管理員重設後，需先更換密碼。</p>{error && <ErrorBox message={error} />}<label className="label">目前密碼</label><input className="mb-4 w-full" type="password" value={current} onChange={(e) => setCurrent(e.target.value)} /><label className="label">新密碼（至少 8 碼）</label><input className="mb-6 w-full" type="password" minLength={8} value={next} onChange={(e) => setNext(e.target.value)} /><button className="btn w-full">儲存新密碼</button></form></main>;
}
