...
const sections = [
  ...
  ["QA 與法規", [["效期登記簿", "QA 專案的 QA 分頁可新增證照與系統效期，並依剩餘天數顯示警示。"], ["CCR", "建立變更管制後依合法狀態逐步流轉；每一步保留歷程，效期確認會建立待辦。"], ["法規動態", "全員可在導覽列查閱並篩選；RA/PV 正職成員與管理員可新增、編輯、刪除。"], ["法規 AI 匯入", "1. 點法規動態的 AI 匯入。2. 預設選單則公告：整份內容匯成一筆，摘要後以 • 條列公告內項目；只有輸入含不同日期或不同公告標題時才選多則彙整。3. 貼文字或上傳 10 MB 內的 .txt／含文字層 .pdf。4. 編輯預覽並排除不要的列。5. 確認匯入；撞鍵資料會略過，原始檔可從條目下載。掃描 PDF 請先改貼文字。"]]],
  ["協作", [["@提及", "1. 留言輸入 @。2. 選姓名。3. 送出後對方收到通知。"], ["檔案", "拖放或選檔上傳；可在專案或任務層級下載與刪除。"], ["自動化", "1. 選觸發條件。2. 選動作。3. 建立後可停用。"]]],
  ["個人工作", [["待辦", "可關聯專案；自動模式下完成會推進專案進度。"], ["通知", "鈴鐺顯示未讀數；點通知可直達專案或任務。"], ["時間軸", "依組別泳道比較可見專案；用頂部選單篩選組別，點 ▶ 展開任務時間條，展開狀態會記在本機。"], ["組別週報與月報", "到報表選組別與本週／上週／本月／上月後產生。一般版本排除保密專案；管理員才可勾選含保密專案。報告右上可列印。"], ["字級", "到個人設定切換標準 17px 或大 18.5px；偏好會記在這個瀏覽器。"]]],
