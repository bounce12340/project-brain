...
export function TimelinePage() {
  ...
  const dates = [currentDate, ...projects.flatMap((project) => [project.start_date, project.target_date, ...project.tasks.flatMap((task) => [task.start_date ?? task.created_at.slice(0, 10), task.due_date]), ...project.events.flatMap((event) => [event.event_date, event.end_date])])].filter((value): value is string => !!value).sort();
