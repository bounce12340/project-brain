...
import { formatQuarter, quarterOfDate, quarterOptions, quarterValue, resolveTargetDate, targetLabel } from "../project-quarter";
...
const INITIAL_SECTIONS = ["core", "updates"];
const LINKABLE_TABS = new Set(baseTabs.map(([key]) => key).concat(["files", "automation"]));
...
export function ProjectDetailPage() {
  ...
  // 總覽最上方的進度快覽要用到進度紀錄，所以一開始就連 updates 一起載入。
  ...
  useEffect(() => { setData(null); setError(""); setSections(INITIAL_SECTIONS); void load(INITIAL_SECTIONS); }, [id]);
  ...
  useEffect(() => {
    ...
    // 快覽裡「寫一則進度」「還有 N 項」直接帶到對應分頁。只接受每種組別都有的分頁。
    else if (linked && LINKABLE_TABS.has(linked)) setTab(linked);
  }, [params]);
...
function Overview({ data, metadata, reload }: { data: ProjectDetail; metadata: Metadata | null; reload(): void }) {
  ...
  const saveSettings = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { target_quarter, ...fields } = submitData(event) as Record<string, string>;
    const target = resolveTargetDate(target_quarter ?? "");
    // 認不得的選項就整個不送 target_date，寧可不動也不要把使用者原本填的洗掉。
    await api(`/projects/${data.project.id}`, patchBody(target === undefined ? fields : { ...fields, target_date: target }));
    reload();
  };
  // 非季末的既有目標日（例如 2026-09-09）在選單裡另列一個選項，
  // 否則打開設定按一次儲存就會把它改成別的日期。
  const quarters = quarterOptions(today());
  const customTarget = data.project.target_date && !quarterOfDate(data.project.target_date) ? data.project.target_date : "";
  const targetQuarter = quarterOfDate(data.project.target_date);
  const targetSelectValue = targetQuarter ? quarterValue(targetQuarter) : customTarget;
  ...
  return <><div className="panel mb-6"><ProjectGlance detail={data} openLink={false} goal={false} /></div>
    <div data-tour="project-overview" className="grid gap-6 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2"><section className="panel"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-bold">{t("project.goal")}<HelpTip topic="projectGoal" /></h2>{data.permissions.can_manage && <label className="flex items-center gap-2 text-sm">{t("project.progressMode")}<HelpTip topic="progressMode" /><select aria-label={t("a11y.progressMode")} value={data.project.progress_mode} onChange={(e) => void api(`/projects/${data.project.id}`, patchBody({ progress_mode: e.target.value })).then(reload)}><option value="manual">{t("project.manual")}</option><option value="auto">{t("project.auto")}</option></select></label>}</div><p className="mt-3 text-sm leading-6 text-star-dim">{data.project.goal_summary || t("project.noGoal")}</p><div className="mt-6"><div className="mb-2 flex justify-between text-sm"><span>{t("project.overallProgress")}<HelpTip topic="progressMode" /> {data.project.progress_mode === "auto" && <span className="text-psi">{t("project.autoHint")}</span>}</span><span className="font-bold">{progress}%</span></div>{data.project.progress_mode === "manual" ? <><input aria-label={t("a11y.progressSlider")} className="w-full" type="range" min="0" max="100" value={progress} disabled={!data.permissions.can_edit} onChange={(e) => setProgress(Number(e.target.value))} />{data.permissions.can_edit && <button className="btn mt-3" onClick={() => void saveProgress()}>{t("project.saveProgress")}</button>}</> : <ProgressBar value={data.project.progress} />}</div></section>
