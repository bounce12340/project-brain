...
function Overview({ data, metadata, reload }: { data: ProjectDetail; metadata: Metadata | null; reload(): void }) {
  ...
  return <><div className="grid gap-6 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2"><section className="panel"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-bold">{t("project.goal")}</h2>{data.permissions.can_manage && <label className="flex items-center gap-2 text-sm">{t("project.progressMode")}<select value={data.project.progress_mode} onChange={(e) => void api(`/projects/${data.project.id}`, patchBody({ progress_mode: e.target.value })).then(reload)}><option value="manual">{t("project.manual")}</option><option value="auto">{t("project.auto")}</option></select></label>}</div><p className="mt-3 text-sm leading-6 text-star-dim">{data.project.goal_summary || t("project.noGoal")}</p><div className="mt-6"><div className="mb-2 flex justify-between text-sm"><span>{t("project.overallProgress")} {data.project.progress_mode === "auto" && <span className="text-psi">{t("project.autoHint")}</span>}</span><span className="font-bold">{progress}%</span></div>{data.project.progress_mode === "manual" ? <><input className="w-full" type="range" min="0" max="100" value={progress} disabled={!data.permissions.can_edit} onChange={(e) => setProgress(Number(e.target.value))} />{data.permissions.can_edit && <button className="btn mt-3" onClick={() => void saveProgress()}>{t("project.saveProgress")}</button>}</> : <ProgressBar value={data.project.progress} />}</div></section>
    ...
    <section className="panel"><h2 className="mb-4 font-bold">{t("project.milestones")}</h2><div className="space-y-2">{data.milestones.map((item) => <label className={`flex items-center gap-3 rounded-lg border p-3 ${!item.done && item.due_date && item.due_date < today() ? "border-danger bg-danger/10" : "border-nexus-line"}`} key={item.id}><input type="checkbox" checked={!!item.done} disabled={!data.permissions.can_edit} onChange={(e) => void toggleMilestone(item.id, e.target.checked)} /><span className={`flex-1 text-sm ${item.done ? "text-star-dim line-through" : ""}`}>{item.title}</span><span className="text-xs text-star-dim">{item.due_date || t("common.notScheduled")}</span></label>)}</div>{data.permissions.can_edit && <form className="mt-4 flex flex-wrap gap-2" onSubmit={addMilestone}><input name="title" placeholder={t("project.newMilestone")} required /><input name="due_date" type="date" /><button className="btn">{t("common.add")}</button></form>}</section></div>
...
function Clinical({ data, reload }: { data: ProjectDetail; reload(): void }) {
  ...
  return <div className="space-y-6"><div className="grid gap-6 lg:grid-cols-3"><section className="panel lg:col-span-2"><h2 className="mb-4 font-bold">{t("clinical.cumulative")}</h2><ResponsiveContainer width="100%" height={300}><LineChart data={chart}><CartesianGrid stroke={CHART.line} strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fill: CHART.starDim }} /><YAxis tick={{ fill: CHART.starDim }} /><Tooltip /><Legend /><Line name={t("dashboard.enrollment")} dataKey="累計收案" stroke={CHART.psi} strokeWidth={3} /><Line name={t("dashboard.target")} dataKey="目標" stroke={CHART.gold} strokeDasharray="4 4" /></LineChart></ResponsiveContainer></section><section className="panel"><p className="text-sm text-star-dim">{t("clinical.current")}</p><p className="my-2 text-4xl font-black text-psi">{total} <span className="text-base text-star-dim">/ {t("common.people", { count: target })}</span></p>{data.permissions.can_edit && <form className="mt-6 flex gap-2" onSubmit={setTarget}><input className="min-w-0 flex-1" name="target_n" type="number" min="0" defaultValue={target} /><button className="btn">{t("clinical.updateTarget")}</button></form>}</section></div>
    {data.permissions.can_edit && <form className="panel grid gap-3 md:grid-cols-5" onSubmit={add}><input name="record_date" type="date" defaultValue={today()} required /><input name="site" placeholder={t("clinical.site")} /><input name="count" type="number" min="0" placeholder={t("clinical.newCount")} required /><input name="note" placeholder={t("common.notes")} /><button className="btn">{t("clinical.record")}</button></form>}
