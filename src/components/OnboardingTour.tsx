...
export function OnboardingTour() {
  ...
  return <div className="fixed inset-0 z-[80] pointer-events-none">{rect && <div className="absolute rounded-xl ring-4 ring-brand-300" style={{ top: rect.top - 6, left: rect.left - 6, width: rect.width + 12, height: rect.height + 12, boxShadow: "0 0 0 9999px rgba(15,23,42,.72)" }} />} {!rect && <div className="absolute inset-0 bg-slate-950/70" />}<div className="pointer-events-auto absolute w-[300px] rounded-xl bg-white p-5 shadow-2xl" style={bubbleStyle}><p className="text-xs font-semibold text-brand-600">導覽 {index + 1} / {steps.length}</p><p className="mt-2 text-sm leading-6">{step.text}</p><div className="mt-4 flex justify-between"><button className="text-sm text-slate-400" onClick={() => void finish()}>略過</button><button className="btn" onClick={() => index === steps.length - 1 ? void finish() : setIndex(index + 1)}>{index === steps.length - 1 ? "完成" : "下一步"}</button></div></div></div>;
}
