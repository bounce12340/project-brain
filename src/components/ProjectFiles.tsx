...
export function ProjectFiles({ projectId, taskId, canEdit, compact = false, onChanged }: { projectId: string; taskId?: string; canEdit: boolean; compact?: boolean; onChanged?(): void }) {
  ...
  return <section className={compact ? "" : "card"}>
    ...
    {canEdit && <div className="mb-4 rounded-xl border-2 border-dashed border-slate-300 p-5 text-center text-sm text-slate-500" onDragOver={(e) => e.preventDefault()} onDrop={drop}><p>{busy ? "上傳中…" : "拖放檔案到此，或選擇檔案（上限 25 MB）"}</p><input ref={input} className="mt-3 max-w-full" type="file" disabled={busy} onChange={(e) => void upload(e.target.files?.[0])} /></div>}
    {files.length ? <div className="divide-y divide-slate-100">{files.map((file) => <div className="flex flex-wrap items-center gap-3 py-3 text-sm" key={file.id}><div className="min-w-0 flex-1"><p className="truncate font-medium">{file.filename}</p><p className="text-xs text-slate-400">{formatBytes(file.size)} · {file.uploaded_by_name} · {formatDate(file.created_at, true)}{file.task_title ? ` · ${file.task_title}` : ""}</p></div><button className="btn-secondary !px-3 !py-1.5" onClick={() => void download(file)}>下載</button>{file.can_delete && <button className="text-xs text-red-500" onClick={() => void remove(file)}>刪除</button>}</div>)}</div> : <Empty>尚無附件</Empty>}
  </section>;
}
