...
export function buildGanttModel({ tasks, milestones, projectStart, projectEnd, currentDate }: {
  ...
  const dated: GanttDatedItem[] = [
    ...
    ...milestones.filter((item) => item.due_date).map((item) => ({ kind: "milestone" as const, item, name: item.title, start: item.due_date as string, end: item.due_date as string })),
  ];
