...
export function Kanban({ projectId, initialStages, initialTasks, canEdit, onReload }: { projectId: string; initialStages: Stage[]; initialTasks: Task[]; canEdit: boolean; onReload(): void }) {
  ...
  return <div><DndContext sensors={sensors} collisionDetection={closestCorners} onDragEnd={onDragEnd}><SortableContext items={stages.map((stage) => `stage:${stage.id}`)} strategy={horizontalListSortingStrategy}><div className="flex min-h-80 gap-4 overflow-x-auto pb-4">{stages.map((stage) => <StageColumn key={stage.id} stage={stage} tasks={tasks.filter((task) => task.stage_id === stage.id).sort((a, b) => a.position - b.position)} canEdit={canEdit} onReload={onReload} />)}</div></SortableContext></DndContext>
...
function StageColumn({ stage, tasks, canEdit, onReload }: { stage: Stage; tasks: Task[]; canEdit: boolean; onReload(): void }) {
  ...
  return <section ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? .6 : 1 }} className="w-72 shrink-0 rounded-xl bg-slate-100 p-3"><header className="mb-3 flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full" style={{ background: stage.color }} /><button type="button" className="flex-1 cursor-grab text-left font-semibold" {...attributes} {...listeners}>{stage.name} <span className="text-xs text-slate-400">{tasks.length}</span></button>{canEdit && <><button className="text-xs text-slate-400" onClick={() => void rename()}>改名</button><button className="text-xs text-red-400" onClick={() => void remove()}>刪</button></>}</header><SortableContext items={tasks.map((task) => `task:${task.id}`)} strategy={verticalListSortingStrategy}><div className="min-h-12 space-y-2">{tasks.map((task) => <TaskCard key={task.id} task={task} canEdit={canEdit} onReload={onReload} />)}</div></SortableContext>{canEdit && <form className="mt-3 flex gap-2" onSubmit={addTask}><input className="min-w-0 flex-1 !px-2 !py-1.5" placeholder="新增工作" value={title} onChange={(e) => setTitle(e.target.value)} /><button className="btn !px-2 !py-1.5">＋</button></form>}</section>;
}

function TaskCard({ task, canEdit, onReload }: { task: Task; canEdit: boolean; onReload(): void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? .5 : 1 }} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm" {...attributes} {...listeners}><div className="flex justify-between gap-2"><p className="text-sm font-medium">{task.title}</p>{canEdit && <button className="text-xs text-red-400" onPointerDown={(e) => e.stopPropagation()} onClick={() => void remove()}>×</button>}</div>{task.assignee_name && <p className="mt-2 text-xs text-slate-500">{task.assignee_name}</p>}{task.due_date && <p className="mt-1 text-xs text-slate-400">到期 {task.due_date}</p>}</article>;
}
