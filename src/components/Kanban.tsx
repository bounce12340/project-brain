...
function TaskCard({ task, canEdit, onReload, onTaskOpen }: { task: Task; canEdit: boolean; onReload(): void; onTaskOpen(task: Task): void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? .5 : 1 }} className={`cursor-pointer rounded-lg border border-slate-200 bg-white p-3 shadow-sm ${task.done ? "opacity-60" : ""}`} onClick={() => onTaskOpen(task)} {...attributes} {...listeners}><div className="flex justify-between gap-2"><p className={`text-sm font-medium ${task.done ? "line-through" : ""}`}>{task.done ? "✓ " : ""}{task.title}</p>{canEdit && <button className="text-xs text-red-400" onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); void remove(); }}>×</button>}</div>{task.assignee_name && <p className="mt-2 text-xs text-slate-500">{task.assignee_name}</p>}{task.due_date && <p className="mt-1 text-xs text-slate-400">到期 {task.due_date}</p>}<p className="mt-2 text-xs text-slate-400">留言 {task.comment_count ?? 0} · 附件 {task.attachment_count ?? 0}</p></article>;
}
