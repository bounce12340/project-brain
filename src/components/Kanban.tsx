...
function TaskCard({ task, canEdit, onReload, onTaskOpen }: { task: Task; canEdit: boolean; onReload(): void; onTaskOpen(task: Task): void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? .5 : 1 }} className={`kanban-task cursor-pointer border border-nexus-line bg-nexus-raised p-3 shadow-sm ${isDragging ? "is-dragging" : ""} ${task.done ? "opacity-60" : ""}`} onClick={() => onTaskOpen(task)} {...attributes} {...listeners}><div className="flex justify-between gap-2"><p className={`text-sm font-medium ${task.done ? "line-through" : ""}`}>{task.done ? "✓ " : ""}{task.title}</p>{canEdit && <button aria-label={t("common.delete")} className="text-xs text-danger" onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); void remove(); }}>×</button>}</div>{task.assignee_name && <p className="mt-2 text-xs text-star-dim">{task.assignee_name}</p>}{task.due_date && <p className="mt-1 text-xs text-star-dim">{t("kanban.due", { date: task.due_date })}</p>}<p className="mt-2 text-xs text-star-dim">{t("kanban.counts", { comments: task.comment_count ?? 0, attachments: task.attachment_count ?? 0 })}</p></article>;
}
