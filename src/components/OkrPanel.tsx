...
export function OkrPanel({ projectId, metadata, onChanged }: { projectId: string; metadata: Metadata | null; onChanged(): void }) {
  ...
  return <section className="panel mt-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold">OKR</h2><p className="text-sm text-star-dim">{t("okr.description")}</p></div><select value={quarter} onChange={(event) => setQuarter(event.target.value)}>{quarterOptions().map((value) => <option key={value}>{value}</option>)}</select></div>
    ...
    {data.can_edit ? <form className="mb-5 flex flex-wrap items-center gap-2" key={`${quarter}-${data.objective?.objective ?? ""}`} onSubmit={saveObjective}><input className="min-w-0 flex-1" name="objective" defaultValue={data.objective?.objective ?? ""} placeholder={t("okr.objectivePlaceholder")} required onChange={() => setSaved(false)} /><button className="btn" disabled={busy}>{t(busy ? "common.processing" : "okr.saveObjective")}</button>{saved && <span className="text-sm text-gold-bright" role="status">{t("okr.saved")}</span>}</form> : <p className="mb-5 text-sm text-star-dim">{data.objective?.objective || t("okr.noObjective")}</p>}
    <div className="mb-2 text-sm font-semibold text-gold-bright">{t("okr.keyResultsLabel")}<HelpTip topic="keyResults" /></div>
