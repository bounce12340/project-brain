...
- 2026-07-31（署名）：作者署名放在登入頁品牌區塊、`app.tagline` 之下。`app.brand` 另一處渲染在 `Layout.tsx` 的導覽列，但那是一行內的連結，加入 15px 粗體整句會擠壞導覽版面，故不放該處。字級依需求固定為 `text-[15px]`；此為絕對像素，不會隨「大字體」偏好（`html[data-fontsize="large"]`）縮放，與週邊使用 `text-sm` 的文字行為不同。
