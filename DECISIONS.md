...
- 2026-09-08（全域時間軸長度不對）：先查正式資料再改。軸線從 2025-11 開始，來源是一筆真實的歷程事件「啟動討論針對檸檬香料廠停產」（2025-11-14，RA：申請矯味劑變更來源）。資料沒錯，錯的是專案軸的畫法把這個離群值放大成災難。
- 2026-09-08（全域時間軸長度不對）：`project.start_date ?? start` 是主因——沒填開始日就退回「整條軸線的最左端」，沒填目標日則讓結束等於開始。正式資料上 **25 個進行中專案的 start_date 全部是 null**，只有兩個有 target_date；於是 23 條專案軸是釘在 2025-11 的 8px 小點，另外兩條是從 2025-11 橫跨到目標日的假長條。改為 `projectSpan()`：沒填的那一端由裡面的任務與里程碑推算，兩端都推不出來就不畫。
- 2026-09-08（全域時間軸長度不對）：`projectSpan` 刻意不採用任務的 `created_at`。那是紀錄被建進系統的時間，不是工作開始的時間；一批專案同時匯入會讓它們全部從匯入那天長出來。既有的任務列仍以 `created_at` 為退路，那是另一個問題（專案內頁甘特有「未排程」分區，全域時間軸沒有），不在這次範圍。
- 2026-09-08（全域時間軸長度不對）：`/timeline` 原本的查詢是 `WHERE kind='event'`——**只抓歷程事件，完全不抓里程碑**。使用者回報的「Apply the PMF for Pathone 沒有起訖」正是這個：那個專案 2 個任務都沒填日期，7 筆有日期的項目全是里程碑，所以它在全域時間軸上真的沒有東西可畫。改為兩種都抓。這擴大了這一頁原本的範圍（頁面說明寫的是「任務起訖、負責人與歷程事件」），但對以送件與核准為節奏的 RA 工作來說，里程碑本來就是排程本身。
- 2026-09-08（全域時間軸長度不對）：欄位名從 `events` 改為 `markers`、`event_date` 改為 `due_date`。里程碑塞進一個叫 `events` 的陣列會讓下一個讀這段程式的人誤判；影響範圍只有 `TimelinePage.tsx` 與端點本身，趁改動時一併正名。
- 2026-09-08（全域時間軸長度不對）：里程碑畫實心、歷程事件維持空心虛線，與專案內頁甘特的視覺語言一致。`tests/v12-4-history-events.test.ts` 的釘選條件跟著改寫——原本比對字面的 `row.kind === "event"` 與 `fill="none"`，改為比對 `row.kind === "marker"` 與 `milestone ? colour : "none"`，守的意圖（兩種標記一眼可分、事件是空心）沒有放寬。
- 2026-09-08（AI 小幫手：建立內容與時程無法輸入）：回報「那頁不能輸入任何的東西」。實測三個頁面：專案內頁有 1 個輸入框且打字正常，儀表板與時間軸則是 0 個。第一版只在網址是 `/projects/:id` 時才渲染表單，其他頁面只留一句「請先開啟一個專案」——分頁在每一頁都點得到，點下去卻是死路，看起來就是壞掉。
- 2026-09-08（AI 小幫手：建立內容與時程無法輸入）：改成在面板裡直接選專案（沿用 `/projects?summary=1`，與專案切換器同一份清單），而不是把使用者趕去別的頁面。`projectId = openProjectId || chosen`——在專案內頁沿用網址那個，其他頁面用選單那個。瀏覽器實測從儀表板選 p2 走完整流程，4 筆寫入全部落在 `/projects/p2/…`。
- 2026-09-08（AI 小幫手：建立內容與時程無法輸入）：測試釘住「不在專案內頁時要有選單」與「寫入用選定的專案而非網址上的」。這個缺陷的形態是「畫面上什麼都沒有」，靠掃描原始碼比靠人記得去別的頁面點一次可靠。
- 2026-09-08（AI 小幫手回「暫時無法回應」）：那句話是 `/ai/assistant` catch 到例外時回的，代表 `llmChat` 丟了例外。但原因被吞掉了——回報的人與要修的人都只看得到同一句廢話，而可能的原因（金鑰無效、模型名不存在、額度用盡、逾時）處理方式完全不同。這是我當初寫這個端點時的盲點。
- 2026-09-08（AI 小幫手回「暫時無法回應」）：`llmChat` 的非 2xx 錯誤改為帶上供應商回的訊息，不再只有 `LLM HTTP 401`。DeepSeek 等 OpenAI 相容端點的錯誤內容會把金鑰遮成 `****`，訊息長度另外設 200 字上限，避免把整份 HTML 錯誤頁塞進日誌。
- 2026-09-08（AI 小幫手回「暫時無法回應」）：管理員在畫面上看得到真正的原因，一般使用者仍只看到「暫時無法回應」。錯誤訊息對管理員是診斷資訊，對其他人只是噪音。
- 2026-09-08（AI 小幫手回「暫時無法回應」）：新增 `GET /ai/diagnostics`（限管理員）。登入後在瀏覽器直接開 `/api/ai/diagnostics` 就會回金鑰在不在、base URL、模型名，以及一次極小的實際往返結果與耗時。只回 `key_present` 布林值，不回金鑰本身。AI 壞掉時最花時間的一直是分辨「金鑰／模型名／逾時」，這個端點一次講完。
- 2026-09-08（AI 小幫手回「暫時無法回應」）：`tests/ai-sidebar.test.ts` 的 `handler()` 原本切到「下一個 `aiRoutes.post(`」。新增 `.get` 端點後，前一個處理函式的切片會一路吃到檔尾，掃描範圍悄悄失準——測試還是綠的，但守的範圍已經不對。改成切到下一個 `aiRoutes.<任何方法>`。另以「故意拿掉管理員檢查」驗證新測試真的會失敗，而不是永遠通過。
- 2026-09-09（專案起訖日可以填了）：後端的 `PATCH /projects/:id` 一直都收 `start_date` 與 `target_date`，但前端從來沒有放這兩個輸入框——設定表單只有產品、廠區、可見性、狀態。這解釋了為什麼 25 個進行中專案的 `start_date` 全是 null：唯一能填目標日的地方是「新增專案」表單，建立之後就再也沒有管道。
- 2026-09-09（專案起訖日可以填了）：預計完成改以「某年第幾季」輸入，但**底層仍存日期**。`target_date` 有 12 處在讀（時間軸、甘特、AI 排程建議、報表、匯入…），改存 `2026Q4` 這種字串會讓它們全部要跟著改，而且各自再解析一次。季度只是輸入與顯示的形式，一律換算成該季最後一天——「第幾季完成」是期限，所以取季末不是季初。
- 2026-09-09（專案起訖日可以填了）：顯示時反向判斷——剛好落在季末的日期顯示成「2026 Q4」，其他日期原樣顯示。這讓既有的兩筆明確目標日（2026-09-09、2026-09-12）不會被誤標成季度，也不需要 migration。
- 2026-09-09（專案起訖日可以填了）：非季末的既有目標日在下拉裡另列成一個選項並預設選取。少了這一步，使用者只要打開設定按一次儲存，原本填的 2026-09-09 就會被改成選單預設的那個季度——一個不改任何欄位卻會改到資料的陷阱。瀏覽器實測「什麼都不改直接儲存」，送出的仍是 2026-09-09。
- 2026-09-09（專案起訖日可以填了）：`resolveTargetDate` 認不得的值回 `undefined`，呼叫端據此整個不送 `target_date`。寧可不動，也不要因為解析失敗而把使用者原本的資料覆蓋成空。
- 2026-09-09（專案起訖日可以填了）：日期驗證不能只看結構。`2026-13-01` 符合 `\d{4}-\d{2}-\d{2}` 但沒有 13 月；改用「轉成 Date 再轉回字串比對」來確認日期真的存在。第一版就是只比對結構，被測試抓到。
- 2026-09-16（儀表板批次刪除）：刪除單一檔案時會呼叫 `store.delete(storageKey)` 清 R2，但**刪除專案完全沒碰 R2**。資料庫的 `ON DELETE CASCADE` 清掉 `files` 那幾列之後，就再也沒有任何紀錄指向那些物件——它們永久留在 bucket 裡繼續計費且無法清理。這是既有缺陷，而一鍵批次刪除會把它放大，所以一併修：刪列之前先清 R2。
- 2026-09-16（儀表板批次刪除）：R2 的清除用 `allSettled` 且失敗只記錄不中斷。某個物件清不掉不該讓整個刪除卡住——資料庫才是使用者眼中的真相，卡住只會讓他們重按一次而已。
- 2026-09-16（儀表板批次刪除）：確認改為打字確認，不是 `window.confirm`。刪除會連帶清掉任務、里程碑、進度紀錄、檔案、OKR、證照、CCR、臨床收案與 BD 費用（全部 `ON DELETE CASCADE`）且無法復原；一個 OK 鈕擋不住手比腦快。確認文字明列會消失什麼，並指出「只是想收起來就用封存」。
- 2026-09-16（儀表板批次刪除）：沿用既有的 `archivableSelection`，不另寫一份刪除專用的選取邏輯。封存與刪除的權限規則本來就相同（管理員或擁有者），後端的 `canManageProject` 也是同一條；寫成兩份只會慢慢分岔。在呼叫處註明這件事，比用兩個名字指同一個概念清楚。
- 2026-09-16（儀表板批次刪除）：沒有在歸檔頁另建一套勾選介面。個別刪除本來就存在於專案內頁，歸檔的專案刪得掉、只是一次一件；使用者要的是批次，而批次勾選介面已經在儀表板上了。
- 2026-09-16（儀表板批次刪除）：瀏覽器實測「打錯字」與「按取消」各送出 0 筆 DELETE，正確輸入才送出。第一次實測把「非擁有者不該有勾選框」判成失敗，實際是測試身分用了 admin——admin 對所有專案都有管理權，出現勾選框才是對的。改用 member 身分重測才驗到真正的權限閘門。
