...
projectsRoutes.post("/", async (c) => {
  ...
  await c.env.DB.prepare(`
    INSERT INTO projects (id, name, description, group_id, owner_id, visibility, goal_summary, start_date, target_date, auto_archive, last_activity_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
projectsRoutes.get("/:id", async (c) => {
  ...
  const [members, stages, tasks, milestones, updates, clinicalSettings, enrollments, cases, events] = await Promise.all([
    ...
    c.env.DB.prepare("SELECT t.*, u.name AS assignee_name FROM tasks t LEFT JOIN users u ON u.id = t.assignee_id WHERE t.project_id = ? ORDER BY t.stage_id, t.position").bind(projectId).all(),
  ...
  return c.json({ project, permissions: { can_edit: canEditProgress(user, access), can_manage: canManageProject(user, access), can_view_fees: canViewFees(user, access) }, members: members.results, stages: stages.results, tasks: tasks.results, milestones: milestones.results, progress_updates: updates.results, clinical_settings: clinicalSettings, enrollments: enrollments.results, bd_cases: cases.results, bd_events: events.results, ...(fees ? { bd_fees: fees.results } : {}) });
});
...
projectsRoutes.patch("/:id", async (c) => {
  ...
  const hasSettings = ["name", "description", "goal_summary", "visibility", "status", "start_date", "target_date", "auto_archive"].some((key) => key in body);
  ...
  await c.env.DB.prepare(`UPDATE projects SET name=?, description=?, goal_summary=?, visibility=?, status=?, progress=?, start_date=?, target_date=?, auto_archive=?, updated_at=CURRENT_TIMESTAMP, last_activity_at=CURRENT_TIMESTAMP WHERE id=?`)
    .bind(optionalString(body, "name") ?? current.name, optionalString(body, "description") ?? current.description, optionalString(body, "goal_summary") ?? current.goal_summary, visibility, status, progress ?? current.progress, optionalString(body, "start_date") ?? current.start_date, optionalString(body, "target_date") ?? current.target_date, "auto_archive" in body ? booleanInt(body, "auto_archive", current.auto_archive) : current.auto_archive, id).run();
