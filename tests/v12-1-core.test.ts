...
import { formatProgressEditedAt, isAutoGeneratedProgressUpdate } from "../src/progress-updates";
...
describe("SPEC-V12-1 UI 與 i18n 契約", () => {
  it("編輯時間固定為台北 YYYY/MM/DD HH:mm", () => {
    expect(formatProgressEditedAt("2026-07-27T08:05:00.000Z", "zh")).toBe("2026/07/27 16:05");
    expect(formatProgressEditedAt("2026-07-27T08:05:00.000Z", "en")).toBe("2026/07/27 16:05");
  });

  it("只有 ✔ 完成前綴顯示 auto 刪除不回滾說明", () => {
    expect(isAutoGeneratedProgressUpdate("✔ 完成任務「Demo」")).toBe(true);
    expect(isAutoGeneratedProgressUpdate("一般人工進度")).toBe(false);
  });

  it("中英 key parity，timeline 有就地 textarea、刪除 icon 與 auto 刪除說明", () => {
    const source = readFileSync(new URL("../src/pages/ProjectDetailPage.tsx", import.meta.url), "utf8");
    expect(Object.keys(en).sort()).toEqual(Object.keys(zh).sort());
    expect(source).toContain("data-progress-update-edit");
    expect(source).toContain("data-progress-update-delete");
    expect(source).toContain('t("project.autoDeleteHint")');
    expect(source).toContain('t("project.editedAt"');
